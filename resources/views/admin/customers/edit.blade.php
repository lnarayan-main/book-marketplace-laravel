    ...
    <form action="{{ route('admin.customers.update', $customer) }}" method="POST">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $customer->name) }}" class="mt-1 block w-full border rounded-md p-2 focus:ring-indigo-500 border-slate-300">
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700">Email Address</label>
                <input type="email" name="email" value="{{ old('email', $customer->email) }}" class="mt-1 block w-full border rounded-md p-2 focus:ring-indigo-500 border-slate-300">
            </div>
            ...
            <div class="flex items-center gap-4 pt-4">
                ...
                <a href="{{ route('admin.customers.index') }}" class="text-slate-600 hover:text-slate-900 text-sm">Cancel</a>
            </div>
        </div>
    </form>
</div>
